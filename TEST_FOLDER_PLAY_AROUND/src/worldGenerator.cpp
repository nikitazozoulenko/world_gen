...
Chunk generateChunk(glm::vec2 chunk_pos);
float totalPerlinValue(int world_x, int world_z);
float lerp(float val0, float val1, float t);
float getPerlinValue(float x, float y, glm::vec2 corner_pos, float amplitude);
int random(glm::vec2 pos);

std::vector<glm::vec2> gradients = {glm::vec2(-1.0, 1.0), glm::vec2(1.0, 1.0), glm::vec2(-1.0, 1.0), glm::vec2(-1.0, -1.0)};
...
{   
    ...
    std::cout << "WG2" << std::endl;

    Chunk chunk = Chunk(chunk_pos, block_array);

    std::cout << "WG3" << std::endl;
    return chunk;
}
...
int random(glm::vec2 pos) //TODO integrate seed
{
